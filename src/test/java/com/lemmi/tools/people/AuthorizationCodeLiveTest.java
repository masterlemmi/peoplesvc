package com.lemmi.tools.people;
...
public class AuthorizationCodeLiveTest {
    ...
    public final static String RESOURCE_SERVER = "http://localhost:8081";
    ...
    public void givenUser_whenUseFooClient_thenOkForFooResourceOnly() {
       ...
       // final Response fooResponse = RestAssured.given().header("Authorization", "Bearer " + accessToken).get(RESOURCE_SERVER + "/api/foos/1");
        final Response fooResponse = RestAssured.given().header("Authorization", "Bearer " + accessToken).get(RESOURCE_SERVER + "/api/people?q=test");
        assertEquals(200, fooResponse.getStatusCode());
        String resp = fooResponse.getBody().prettyPrint();
        assertNotNull(resp);
